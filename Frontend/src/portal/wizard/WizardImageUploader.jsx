  ...
  return (
    <div className="wizard-uploader">
      <label className="wizard-uploader__dropzone">
        ...
        <span className="wizard-uploader__hint">JPG/PNG, multiple files allowed</span>
      </label>
        ...
        <ul className="wizard-uploader__pending">
          {pending.map((f) => (
            <li key={f.name} className={f.status === "error" ? "is-error" : ""}>
              {f.name} — {f.status === "error" ? f.error || "Upload failed" : "Uploading..."}
            </li>
          ))}
        </ul>
      )}
        ...
        <div className="wizard-uploader__grid">
          {attachments.map((a) => (
            <div key={a.id} className="wizard-uploader__item">
              <img src={a.url} alt={a.originalFileName || "uploaded"} />
              <textarea
                placeholder="Note for this image (optional)"
                value={a.note || ""}
